...
void Read_input()
{
	...
	struct indi_fleet_size *head_ifs, *indi_fleet_size, *last_ifs;//NEU_DOC
	struct hc *head_hc, *hc, *last_hc;//NEU_DOC
  ...
  */  
	...
	for (i=0;i<ACTYPE_NO;i++)
	{
		...
		head_ifs=NULL;
		indi_fleet_size=(struct indi_fleet_size *)malloc(sizeof(struct indi_fleet_size));
			fscanf(fp,"%d ",&indi_fleet_size->ifs); //NEU_MAC
			indi_fleet_size->next_ifs=NULL;
			if(head_ifs==NULL)
			{
				head_ifs=indi_fleet_size;
				last_ifs=indi_fleet_size;
			}
			else
			{
				last_ifs->next_ifs=indi_fleet_size;
				last_ifs=last_ifs->next_ifs;
			}
	fleet_size+=indi_fleet_size->ifs;
    }	 
	//reading the comment lines
    for(i=0;i<2;i++)
    {    
    ...
    for(i=0;i<fleet_size;i++)
    {
        ...
        fscanf(fp,"%d %d %s %f %f %f %f",&fleet->ac_index,&fleet->ac_type,fleet->ap_name,&fleet->takeoff_time,&fleet->life,&fleet->BH_max,&fleet->TAT); //NEU_MAC NEU_BH (ac_type,BH_max,BH_remain)
		fleet->BH_remain=fleet->BH_max; //NEU_BH
		tat->tat=fleet->TAT;
		//tat[fleet->ac_index]=fleet->TAT;
    ...
    */
	...
	for(i=0;i<total_bases;i++)
	{
		...
		head_hc=NULL;
		fscanf(fp,"%d %s %d",&base->base_index,base->base_name,&base->hc);
		base=base->next_base;
	...
	for(i=0;i<total_bases;i++)
    {
		...
		{
			hc=(struct hc *)malloc(sizeof(struct hc));
			fscanf(fp," %d",&hc->cap);
			hc->next_hc=NULL;
			if(head_hc==NULL)
			{
...
*/
    ...
    while(base!=NULL)
    {
        ...
        for(j=0;j<ACTYPE_NO;j++){
		   tc=(struct tc *)malloc(sizeof(struct tc));
        tc->tcap=hc->cap
        tc->next_tc=NULL;
        if(head_tc==NULL)
        {
        ...
        {
            last_tc->next_tc=tc;
            last_tc=last_tc->next_tc;
        }
		hc=hc->next_hc;}
        base=base->next_base;
    }
    for(j=0;j<ACTYPE_NO;j++)
	if(fleet_size_indi[j]>total_capacity[j])
    {
        printf("\n Error : Total base capacity for aircraft type %d is less then the fleet size",j+1);
        getch();
        exit(1);
    }        
    //reading the comment lines
