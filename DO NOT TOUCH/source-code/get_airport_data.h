...
int Get_airport_data(struct Fleet *candidate)
{
    ...
    if(best_index==-1)  
    {
        ...
        if(user_choice ==YES)
        {
            ...
            for(i=0;i<AIRPORT_NO;i++)
				fprintf(fprocess,"\n from airport %d = %.2f, %.2f, %.2f",i+1,ap_demand[i],ap_capacity[i],BELF_each[i]);
		...
		while(data!=NULL)
		{
			...
			while(data!=NULL && j<AIRPORT_NO)
			{
				...
				if (data->demand >= (temp_capacity*BELF_each[i]) && temp_capacity != 0) //NEU_MAC
				ap_with_belf[i]++;
				data=data->next_data;
				j++;
			}
