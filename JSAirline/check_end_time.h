...
int Check_end_time(struct Fleet *candidate, float new_take_off, int desti_index, float time_diff)
{
    ...
    for(i=0;i<total_bases;i++)
    {
        ...
        temp_head=candidate;
         ...
         {
			    for(j=0;j<ACTYPE_NO;j++)
             fprintf(fprocess,"\n Total %d aircraft of type %d are available on base airport %d",base_aircraft[j],j+1,base->base_index);
             fclose(fprocess);
		   }  
		 hc=base->hc;
		 for (j=0;j<candidate->ac_type-1;j++)
hc=hc->next_hc;
        ...
        if(hc->cap - base_aircraft[candidate->ac_type-1] >0)
        {
